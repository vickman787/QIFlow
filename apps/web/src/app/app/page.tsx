...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div className="text-xs text-[#8B9CC8] font-medium uppercase tracking-wider">{label}</div>
      {sub && <div className="text-xs text-[#8B9CC8] mt-0.5">{sub}</div>}
    </div>
  );
}
      ...
      <div className="bg-[#131B3D] border border-[#B7791F]/20 rounded-2xl p-5">
        ...
        <div className="space-y-2">
          {networkFields.map(({ label, value }) => (
            <div
              key={label}
              className="flex items-center justify-between gap-3 bg-[#0D1535] rounded-xl px-4 py-3"
            >
              <div className="min-w-0 flex-1">
                <p className="text-[10px] text-[#8B9CC8] font-semibold uppercase tracking-wider mb-0.5">
                  {label}
                </p>
